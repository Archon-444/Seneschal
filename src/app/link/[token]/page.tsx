...
export default async function ExternalLinkPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (!validation.ok) {
    return (
      <SafeShell>
        <h1 className="text-xl font-semibold text-navy-900">This link is no longer available</h1>
  ...
  if (validation.link.purpose === "LISTING_VIEW") {
    ...
    if (!listing) {
      return (
        <SafeShell>
          <h1 className="text-xl font-semibold text-navy-900">This listing is no longer available</h1>
        </SafeShell>
      );
    }
    ...
    return (
      <SafeShell>
        <div className="flex items-start justify-between gap-3">
          <h1 className="text-xl font-semibold text-navy-900">{listing.headline ?? unit}</h1>
        ...
        <dl className="mt-5 divide-y divide-ivory-200 rounded border border-ivory-300">
        ...
        <div className="mt-7 border-t border-ivory-200 pt-5">
          <h2 className="text-[15px] font-semibold text-navy-900">Interested?</h2>
        ...
        <div className="mt-8 rounded bg-ivory-100 p-4 text-xs leading-relaxed text-navy-500">
  ...
  if (validation.link.purpose === "PASSPORT_SHARE") {
    ...
    if (!p) {
      return (
        <SafeShell>
          <h1 className="text-xl font-semibold text-navy-900">This passport is no longer available</h1>
        </SafeShell>
      );
    }
    ...
    return (
      <SafeShell>
        <h1 className="text-xl font-semibold text-navy-900">{p.tenantName}</h1>
        ...
        {p.documentKinds.length > 0 && (
          <div className="mt-5">
            <div className="text-[12px] font-medium text-navy-700">Documents provided</div>
  ...
  if (validation.link.purpose === "TENANT_OFFER") {
    ...
    return (
      <SafeShell>
        <h1 className="text-xl font-semibold text-navy-900">Renewal proposal</h1>
  ...
  if (validation.link.purpose === "APPROVAL") {
    ...
    return (
      <SafeShell>
        <h1 className="text-xl font-semibold text-navy-900">Owner sign-off</h1>
  ...
  return (
    <SafeShell>
      <h1 className="text-xl font-semibold text-navy-900">{request.title}</h1>
...
function SafeShell({ children }: { children: React.ReactNode }) {
  return (
    <main className="min-h-screen bg-ivory-100 px-4 py-10">
      <div className="mx-auto max-w-lg rounded border border-ivory-300 bg-white p-6">
        <div className="mb-6 text-[15px] font-semibold text-navy-300">Seneschal · secure upload</div>
        {children}
      </div>
    </main>
  );
