...
export default async function CalendarPage({
  ...
  const all = await listDeadlines(ctx, kind ? { kind: kind as never } : undefined);
            ...
            <div key={i} className={`min-h-20 border-b border-r border-ivory-200 p-1.5 ${isToday ? "bg-gold-300/10" : ""}`}>
              {day && (
                <>
                  ...
                  {items.map((d) => (
                    <div key={d.id} className="mt-1 truncate rounded bg-navy-50 px-1 py-0.5 text-[10px] text-navy-700" title={d.kind}>
                      {d.kind.replace(/_/g, " ")}
                    </div>
                  ))}
                </>
            ...
            <Table headers={["Due", "Kind", "Property"]}>
              {overdue.map((d) => (
                <tr key={d.id} className="bg-claret-100/40">
                  ...
                  <Td>{d.tenancy?.property ? `${d.tenancy.property.community} · ${d.tenancy.property.unitNo ?? ""}` : "—"}</Td>
                </tr>
              ))}
            </Table>
          )}
