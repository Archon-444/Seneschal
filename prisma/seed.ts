...
import { generateToken, sha256Hex } from "../src/server/crypto";
import { toUtcDateOnly } from "../src/server/calculators/dates";
import { regenerateDeadlinesForTenancy } from "../src/server/services/deadlines";
import { evaluateRiskForTenancy } from "../src/server/services/risk";
import { newStorageKey, storage } from "../src/server/storage";
...
async function main() {
  ...
  if (!existingLink) {
    ...
    console.log(`\nSeeded live proof-upload link: ${process.env.APP_BASE_URL ?? "http://localhost:3000"}/link/${token}\n`);
  }
